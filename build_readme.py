...
def fetch_weekly():
    return httpx.get("https://iT-Boyer.github.io/review/habit.txt")
...
def fetch_blog_entries():
    entries = feedparser.parse("https://it-boyer.github.io/index.xml")["entries"]
    return [
        {
            "title": entry["title"],
            "url": entry["link"].split("#")[0],
            "published": formatDateStr(
                entry["published"]
...
if __name__ == "__main__":
    ...
    # dariy_text = "\n" + paragraphs[0]
    # rewritten = replace_chunk(readme_contents, "dairy", dariy_text)

    # weekly_text = "\n" + paragraphs[1]
    # rewritten = replace_chunk(rewritten, "weekly", weekly_text)

    # read_text = "\n" + paragraphs[2]
    # rewritten = replace_chunk(rewritten, "reader", read_text)

    # writer_text = "\n" + paragraphs[3]
    # rewritten = replace_chunk(rewritten, "writer", writer_text)
